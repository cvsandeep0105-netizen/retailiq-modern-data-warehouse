...
def test_required_schemas_exist(conn):
    cur=conn.cursor()
    cur.execute("""SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name IN ('raw','staging','intermediate','warehouse','marts','semantic')""")
...
def test_analytical_outputs(conn):
    cur=conn.cursor()
    assert scalar(cur,"SELECT COUNT(*) FROM semantic.bi_sales_overview")==99441
    cur.execute("""SELECT COUNT(*) FROM (SELECT DATE_TRUNC('month',order_purchase_timestamp)::date FROM semantic.sales_analysis GROUP BY 1)x""")
    assert cur.fetchone()[0]==25

"""
Set-Content -Path "tests\integration\test_platform_regression.py" -Value $tests -Encoding utf8

$env:PYTHONPATH="$PWD\src"
python -m pytest -q
if ($LASTEXITCODE -eq 0) { Write-Host "PASS - AUTOMATED TESTING AND REGRESSION VALIDATED" } else { Write-Host "FAIL - AUTOMATED TESTING OR REGRESSION FAILED" -ForegroundColor Red; exit 1 }
